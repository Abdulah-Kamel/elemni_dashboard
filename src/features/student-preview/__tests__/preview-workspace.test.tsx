...
function renderWorkspace(props: Partial<React.ComponentProps<typeof PreviewWorkspace>> = {}) {
  return render(
    <PreviewWorkspace
      editor={<EditorSlot />}
      preview={<PreviewSlot />}
...
describe("PreviewWorkspace", () => {
  ...
  it("mounts each slot exactly once — no duplicate desktop/mobile subtrees", () => {
    ...
    expect(container.querySelectorAll("[data-testid='editor-pane']")).toHaveLength(1);
    expect(container.querySelectorAll("[data-testid='preview-pane']")).toHaveLength(1);
  });
  ...
  describe("compact tabs", () => {
    ...
    it("marks the edit tab selected by default", () => {
      ...
      expect(
        screen.getByRole("tab", { name: "معاينة" }).getAttribute("aria-selected")
      ).toBe("false");
    });
    ...
    it("uses a roving tabindex", () => {
      renderWorkspace();
      expect(screen.getByRole("tab", { name: "تعديل" }).getAttribute("tabindex")).toBe("0");
      expect(screen.getByRole("tab", { name: "معاينة" }).getAttribute("tabindex")).toBe("-1");
    });
    ...
    it("moves selection with the End key", () => {
      renderWorkspace();
      fireEvent.keyDown(screen.getByRole("tab", { name: "تعديل" }), { key: "End" });
      expect(
        screen.getByRole("tab", { name: "معاينة" }).getAttribute("aria-selected")
      ).toBe("true");
    });
    ...
    it("moves selection with arrow keys, mirrored for RTL", () => {
      ...
      fireEvent.keyDown(screen.getByRole("tab", { name: "تعديل" }), { key: "ArrowLeft" });
      expect(
        screen.getByRole("tab", { name: "معاينة" }).getAttribute("aria-selected")
      ).toBe("true");
    });
    ...
    it("moves selection with arrow keys in LTR", () => {
      ...
      fireEvent.keyDown(screen.getByRole("tab", { name: "Edit" }), { key: "ArrowRight" });
      expect(
  ...
  describe("device width controls", () => {
    ...
    it("defaults to full device width", () => {
      renderWorkspace();
      expect(screen.getByRole("radio", { name: "كامل" }).getAttribute("aria-checked")).toBe(
        "true"
      );
    });
    ...
    it("constrains the rendered preview, not the toolbar", () => {
      ...
      const toolbar = screen.getByRole("radio", { name: "كامل" }).closest("[role='radiogroup']");
  ...
  describe("theming", () => {
    ...
    it("uses logical inline properties, never physical ml/mr/left/right", () => {
      expect(componentSource).not.toMatch(/\b(ml|mr|pl|pr|border-l|border-r|left|right)-/);
    });
  });
});
