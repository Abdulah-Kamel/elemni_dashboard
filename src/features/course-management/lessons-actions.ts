...
import type { LessonOut, LessonCreate, LessonUpdate } from "@/features/course-management/lessons-schema";
import { reorderItemSchema, type ReorderItem } from "@/features/course-management/chapters-schema";
...
export async function reorderLessons(
  ...
  logger.action("reorderLessons", { courseId, chapterId, itemCount: body.length });
  const start = performance.now();
  ...
  try {
    ...
    revalidateTag(`lessons:${courseId}`, "default");
    const elapsed = Math.round(performance.now() - start);
    logger.actionDone("reorderLessons", { lessonIds: lessons.map((l) => l.id) }, elapsed);
    return { success: true, data: lessons };
  } catch (err: unknown) {
    const elapsed = Math.round(performance.now() - start);
    if (err && typeof err === "object" && "type" in err) {
