...
import { chapterCreateSchema, chapterUpdateSchema, reorderItemSchema } from "@/features/course-management/chapters-schema";
import { logger } from "@/lib/logger";
import type { ChapterOut, ChapterCreate, ChapterUpdate, ReorderItem } from "@/features/course-management/chapters-schema";
...
export async function reorderChapters(
  ...
  try {
    ...
    revalidateTag(`chapters:${courseId}`, "default");
    const elapsed = Math.round(performance.now() - start);
    logger.actionDone("reorderChapters", { chapterIds: chapters.map((c) => c.id) }, elapsed);
