...
import { getCourseAction, updateCourse, listSubjectsAction, listGradesAction, listStreamsAction } from "@/features/course-management/actions";
...
type DialogState = "idle" | "loading" | "submitting" | "validation-error" | "conflict" | "network-error";
...
export function EditCourseDialog({
  ...
  }, [formValues, courseId, teacherProfileId]);
        ...
        <div className="space-y-6">
          {state === "loading" ? (
            ...
            <>
              ...
              {state === "conflict" && (
                <p className="text-sm text-destructive">{networkError ?? t("conflict")}</p>
              )}
              {state === "network-error" && (
