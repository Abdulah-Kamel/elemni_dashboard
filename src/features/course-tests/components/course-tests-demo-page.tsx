...
import type { CourseTest, TestStats } from "@/features/course-tests/types";

const emptyStats: TestStats = { published_tests: 0, draft_tests: 0, pending_grading: 0, average_score: null, pass_rate: null };
function isFuture(value: string | null) { return Boolean(value && new Date(value).getTime() > Date.now()); }

export function CourseTestsDemoPage({ courseId, locale }: { courseId: number; locale: string }) {
  const t = useTranslations("courseTests");
  const [tests, setTests] = useState<CourseTest[]>([]);
  const [stats, setStats] = useState(emptyStats);
  useEffect(() => {
    const update = () => { setTests(listCourseTests(courseId)); setStats(getCourseTestStats(courseId)); };
    update();
  ...
  return <div className="space-y-6" dir={locale === "ar" ? "rtl" : "ltr"}>
    <div className="rounded-lg border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-900">وضع العرض التجريبي: تُحفظ تغييرات الاختبارات والتصحيح محلياً في هذا المتصفح.</div>
    <header className="flex flex-wrap items-end justify-between gap-4"><div><nav aria-label={t("breadcrumb")} className="mb-2 text-sm text-muted-foreground"><Link href={`/courses/${courseId}`}>{t("course")}</Link> / {t("title")}</nav><h1 className="text-3xl font-bold tracking-tight">{t("title")}</h1><p className="mt-1 text-muted-foreground">{t("description")}</p></div><div className="flex flex-wrap gap-2"><NewTestButton courseId={courseId} label={t("new_test")} /><Button variant="outline" render={<a href={templateHref} download="course-tests-template.csv" />}>{t("download_template")}</Button></div></header>
